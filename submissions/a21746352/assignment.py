...
def load_corpus_data(file_name):
    ...
    for sentence in sentences:
        ...
        if sentence == " ":
            continue
        data.append(extract_words(sentence))
    # -----
    return data
...
def top_k_words(wfd, k):
    ...
    top_k = []
    # -----
    for i, v in sorted(wfd.items(), key=lambda x: -x[1]):
        top_k.append(str(i))
    top_k = top_k[:k]
    # -----
    return top_k
...
def w_i(data, word, width):
    ...
    for d in data:
        if word not in d:
            ...
            index = d.index(word)
            if index<width:
                addwords=d[0:(index+width+1)]
            elif index>len(d)-width:
                addwords=d[(index-width):index+1]
            else:
                addwords=d[(index-width):(index+width+1)]
            for addword in addwords:
                features.add(addword)
            if word in features:
...
def w_ij(data, word, width):
    ...
    for d in data:
        ...
        if word not in d:
            ...
            if word in draft:
                draft.remove(word)
            draft=set(itertools.combinations(draft,2))
            features=set.union(features,draft)
    # -----
    return features
...
def context(data, word, width):
    ...
    context = set()
    # -----
    features_i=w_i(data, word, width)
    features_ij=w_ij(data, word, width)

    context=set()
    context=set.union(features_i,features_ij)
    # -----
    return context
...
def similarity(cd, w1, w2):
    ...
    if w1==w2:
        ...
        sim=2*(len(set.intersection(cd[w1],cd[w2])))/float(len(cd[w1])+len(cd[w2]))
            #Do not use set.union to avoid erase duplication of the same words
    # -----
    return sim
    ...
    for i in cd.keys():
            ...
            for t in cd.keys():
                if i == t:
                    ...
                    sim_dict.update({t:similarity(cd, i, t)})
            sim_dict = sorted(sim_dict, key=sim_dict.get, reverse=True)
            #list
            knng.update({i:sim_dict[:k]})
    # -----
    return knng
...
def get_normalized_laplacian(sim_matrix):
    ...
    for row in range(len(sim_matrix)):
        D[row ,row]= sum(sim_matrix[row])*sim_matrix[row,row]
    lap = D - sim_matrix
    inv = np.linalg.inv(D)
    l = np.matmul(lap,inv)
    # -----
