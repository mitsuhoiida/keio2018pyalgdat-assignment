...
def extract_words(sentence):
    ...
    # YOUR CODE GOES HERE
    # -----
    #corpus = "Let's strip this sentence! :)"
    sentence = sentence.lower()
    corpus_list = sentence.split(" ")

    for item in corpus_list:
        #print item.split("'")[0]
        tmp = re.compile("[a-z]+").findall(item.split("'")[0])
        if len(tmp)>0:
            words.append(tmp[0])
    return words
...
def get_word_freq(data):
    ...
    for sentence in data:
        for word in sentence:
            #wfd: {"the":1,"fulton":1}
            #word : "the"
            count = wfd.get(word, 0) #1
            wfd[word] = count + 1
            #wfd["the"] = 1
            #wfd: {"the":1, "fulton":3, "a":6, "only":1 }
    return wfd
...
def top_k_words(wfd, k):
    ...
    # -----
    #print wfd.items()[3]
    items = [(val,key) for key, val in wfd.items()]
    #print items
    #print "--------------------------------"
    items.sort()
    #print items
    items.reverse()
    #print items
    items = [key for val,key in items]
    #print items
    #print k
    top_k = items[0:k] #=>0~k-1
    return top_k
...
def w_i(data, word, width):
    ...
    """
    # data = [['extract', 'unigram', 'features', 'from', 'text'], idx 0~4 len 5
    #         ['another', 'fake', 'sentence', 'from', 'corpus', 'data']]
    # data[1] = ['another', 'fake', 'sentence', 'from', 'corpus', 'data']
    features = set()
    for sentence in data:
        if word in sentence:
            ...
            continue
        # 3 => 3-2 => 1~5 <=3+2
        # sentence[idx-width:idx+width+1] => [1:6] 1,2,3,4,5
        ...
        if pre_idx < 0:
            ...
            is_self = 1
        post_idx = idx + width
        if post_idx>=len(sentence):
            post_idx = len(sentence) - 1
            is_self = 1
           
        result_list = sentence[pre_idx:idx]
        result_list2 = sentence[idx+1:post_idx+1]
        #print result_list
        #print result_list2
        ...
        #print result_list
        features = features | set(result_list) #intersection &
    # -----
    # YOUR CODE GOES HERE
    # -----
    #alist = ["a","a","b"]
...
def w_ij(data, word, width):
    ...
    for sentence in data:
        ...
        features = features | set(itertools.combinations(result_list,2)) #intersection &
...
def context(data, word, width):
    ...
    """
    context = set()
    unigram_features = w_i(data,word,width)
    bigram_features = w_ij(data,word,width)
    context = unigram_features | bigram_features
    # -----
    # YOUR CODE GOES HERE
    # -----
...
def get_context_dictionary(data, words, width):
    ...
    # -----
    for word in words:
        # print(data,word,width)
        cd[word] = context(data, word, width)
        # print(tmp)
        # cd[word] = tmp
...
ddef similarity(cd, w1, w2):
    ...
    if w1 == w2:
        ...
        set_w1 = cd[w1]
        set_w2 = cd[w2]
        if len(set_w1) == 0 or len(set_w2) == 0:
            sim = 0.0
        else:
            intersect_set = set_w1.intersection(set_w2)
            union_set = set_w1.union(set_w2)
            sim = 2*(len(intersect_set)/len(union_set))
    return sim
...
def get_knn_graph(cd, k):
    ...
    # YOUR CODE GOES HERE
    # -----
    words = set(list(cd.keys()))
    #i = 0
    for subset in set(itertools.combinations(set(words), 2)):
        #print(i)
        #i = i + 1
        #print(subset)
        w1 = subset[0]
        w2 = subset[1]
        sim_score = similarity(cd, w1, w2)
        #print(sim_score)
        if w1 not in knng.keys():
            knng[w1] = [(w2, sim_score)]
        else:
            knng[w1].append((w2, sim_score))

        if w2 not in knng.keys():
            knng[w2] = [(w1, sim_score)]
        else:
            knng[w2].append((w1, sim_score))
    # print(knng)
    for key in knng.keys():
        knng[key].sort(key=lambda tup: tup[1], reverse=True)
        knng[key] = knng[key][0:k]
        #knng[key] = [key for key, val in knng[key]][0:k]
    return knng
...
if __name__ == "__main__":
    print("------------------------------------")
    data2 = [['extract', 'unigram', 'features', 'from', 'text'],
    ['another', 'fake', 'sentence', 'from', 'corpus', 'data']]
    features2 = w_ij(data2, 'from', 2)
    print(features2)
    print(len(features2))

    print("------------------------------------")
    context_result = context(data2, 'from', 2)
    print(context_result)

    print("------------------------------------")
    cd = get_context_dictionary(data2, ['from','data','corpus'], 2)
    print(cd)


    print("------------------------------------")
    sim = similarity(cd, "from", "data")
    print(sim)

    print("------------------------------------")
    knng = get_knn_graph(cd, 1)
    print(knng)
