...
def top_k_words(wfd, k):
    ...
    for i in wfd:
        top_k.append((wfd[i], i))
    tmp = sorted(top_k)
    top_k = []
    for i in range(1, k + 1):
        top_k.append(tmp[-i][1])
...
def w_ij(data, word, width):
    ...
    for s in data:
        if word in s:
            positions = [x for i, x in enumerate(s) if x != word]
            p = s.index(word)
            a = (positions[p - width: p + width + 1])
...
def context(data, word, width):
    ...
    uset = w_i(data, word, width)
    bset = w_ij(data, word, width)
    context = (set(uset) | set(bset))
...
def similarity(cd, w1, w2):
    ...
    if w1 == w2:
        ...
        c1 = cd[w1]
        c2 = cd[w2]
        if len(c1) == 0 or len(c1) == 0:
            sim = 0.0
        else:
            sim = 2.0*len(c1.intersection(c2))/(len(c1)+len(c2))
...
def get_knn_graph(cd, k):
    ...
    for key1 in cd:
    	...
    	for key2 in cd:
    		if key1 != key2:
    			z[key2] = similarity(cd, key1, key2)
    	#temp = [j[0] for j in sorted(z, key=(lambda x: x[1]), reverse = True)][:k] (tried my best here, but the list is empty if i use lambda:())
    	sort = sorted(z, key = z.get, reverse = True)
    	temp = sort[:k]
    	knng[key1] = temp
    return knng
...
def get_sim_matrix(cd):
    ...
    for i in range(0, len(cd.keys())):
        for j in range(0, len(cd.keys())):
            sim_matrix[i, j] = similarity(cd, list(cd)[i], list(cd)[j])
        word_index[list(cd)[i]] = i
