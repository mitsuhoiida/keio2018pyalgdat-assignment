...
def load_corpus_data(file_name):
    ...
    # -----
    with open("/Users/sam/python local repository/linguistic manifold/data/english-brown.txt") as f:
        for l in f:
...
def get_word_freq(data):
    ...
    # -----
    z = []
    data = load_corpus_data("/Users/sam/python local epository/linguistic Manifold/data/data.pkl")
    for sublist in data:
        for item in sublist:
            z.append(item)
    for w in z:
...
def top_k_words(wfd, k):
    ...
    # -----
    a = sorted(wfd, key = lambda k: wfd[k], reverse = True)
    for i in range(0,k):
        top_k.append(a[i])
        i = i + 1
    # -----
...
def graph_top_k(wfd, top_k, file_name):
    ...
    # -----
    fs = [wfd[k] for k in wfd]
    fs.sort(reverse = True)
    slicelist = slice(len(top_k))
    fs = fs[slicelist]
    # -----
...
def w_i(data, word, width):
    ...
    for sentence in data:
        m =len(sentence)
        if word in sentence:
            z = [i for i,x in enumerate(sentence) if x == word]
            for index in z:
                if index-width >= 0 and index+width < m:
                    feature = sentence[index-width:index+width+1]
                elif index-width < 0 and index+width < m:
                    feature = sentence[:index+width+1]
                elif index-width < 0 and index+width >= m:
                    feature = sentence[:]
                else:
                    feature = sentence[index-width:]
                feature.remove(word)
                featuressum.extend(feature)
    features = set(featuressum)
    # -----
        ...
        if word in sentence:
            ...
            for index in z:
                if index-width >= 0 and index+width < m:
                    ...
                    feature = sentence[index-width:]
                feature.remove(word)
                combination.extend(itertools.combinations(feature, 2))
    features = set(combination)
    # -----
...
def context(data, word, width):
    ...
    # -----
    context = w_i(data,word,width).union(w_ij(data,word,width))
    # -----
...
def get_context_dictionary(data, words, width):
    ...
    k = 0
    for c in words:
        cd[c] = context(data,c, width)
        k = k + 1
        k_per = int((k/length)*100)
        if k % 40 == 0:
            print(k, "/", length, k_per, "% completed")
    # -----
...
def similarity(cd, w1, w2):
    ...
    if w1 == w2:
        ...
        sim = 2 * float(len(same) / disjointunion)
    # -----
...
def get_knn_graph(cd, k):
    ...
    for w1 in cd:
        neighbor = []
        y = [(w2, similarity(cd,w1,w2)) for w2 in cd if w1 != w2]
        sortedlist = sorted(y, key = lambda x:x[1], reverse = True)[:k]
        neighbor = [x[0] for x in sortedlist]
        knng[w1] = neighbor
        f = f + 1
        f_per = int((f/41217)*1000)
        if f % 40 == 0:
            print(f, "/", 41217, f_per, "%% completed")
    # -----
...
def get_sim_matrix(cd):
    ...
    for w1 in cd:
        word_index[w1] = k
        k = k + 1
        q.append([similarity(cd,w1,w2) for w2 in cd])
        m = m + 1
        if m % 20 == 0:
            print(m, "/", leng)
    sim_matrix = np.array(q)
    # -----
...
def get_normalized_laplacian(sim_matrix):
    ...
    # -----
    diagonal = sim_matrix.sum(axis=1)
    degree = np.diag(diagonal)
    invdegree = np.linalg.inv(degree)
    l = np.matmul(invdegree,(degree - sim_matrix))
    # -----
...
def get_eigenvector_matrix(l, k):
    ...
    # -----
    s = list(np.linalg.eig(l))[1:]
    q = s[0]
    v = np.array([x[:k] for x in q])
...
def word_distance(w1, w2, wv):
    ...
    #
    # Output:
    # dist - a floating point number in the range (0, 2)
    #
    # Instructions:
