...
def load_corpus_data(file_name):
    ...
    for sentence in data:
        data[data.index(sentence)] = extract_words(sentence)
    # -----
...
def top_k_words(wfd, k):
    ...
    # -----
    sort = sorted(wfd, key = wfd.get, reverse = True)
    for i in range(k):
        top_k.append(sort[i])
    # -----
...
def w_i(data, word, width):
    ...
    # -----
    for sentence in data:
        positions = [i for i, x in enumerate(sentence) if x == word]
        for position in positions:
            for i in range(position-width, position+width+1):
                if i<0 or i > len(sentence)-1 or i==position:
                    continue
                features.add(sentence[i])
    # -----
    ...
    for sentence in data:
        ...
        for position in positions:
            ...
            for i in range(position-width,position+width+1):
                if i<0 or i>len(sentence)-1 or i==position:
                    continue
                uni.append(sentence[i])
            elements = itertools.combinations(uni,2)
...
def context(data, word, width):
    ...
    # -----
    uni = w_i(data, word, width)
    bi = w_ij(data, word, width)
    context = uni | bi
    # -----
...
def get_context_dictionary(data, words, width):
    ...
    for word in words:
        ...
        if i % n == 0:
            print('Progress: {:03d}% Complete'.format(i//n))
        i+=1
    # -----
...
def get_knn_graph(cd, k):
    ...
    for word in cd:
        ...
        for comp in cd:
            if word != comp:
                sim_words[comp] = similarity(cd,word,comp)
        print(sim_words)
        sort = sorted(sim_words, key = sim_words.get, reverse = True)
        sort1 = sort[:k]
        knng[word] = sort1
        if i % n == 0:
            print('Progress: {:03d}% Complete'.format(i//n))
...
def get_sim_matrix(cd):
    ...
    for w1 in cd:
        ...
        j=0
        for w2 in cd:
            sim_matrix[i,j] = similarity(cd,w1,w2)
            j+=1
        i+=1
    # -----
...
def get_normalized_laplacian(sim_matrix):
    ...
    i=0
    while i < len(sim_matrix):
        d[i,i] = np.sum(sim_matrix[i])
        i+=1
    l = np.matmul(np.linalg.inv(d), d-sim_matrix)
    # -----
