...
def extract_words(sentence):
    ...
    # ----- 
    # tokens = re.sub("[^\w]", " ", sentence.strip()).split()   does not take out numbers!
    tokens = re.sub('[^a-zA-Z]', " ", sentence.strip()).split()
    words = [w.lower() for w in tokens if len(w) > 1]


    # n = re.sub("'\w+","",sentence)
    # words = re.findall('\w+', n)
    # -----
...
def top_k_words(wfd, k):
    ...
    # -----
    sorted_list = sorted(wfd, key=wfd.get, reverse=True)
    i =0
    while i < k:
...
def graph_top_k(wfd, top_k, file_name):
    ...
    # part of the code replacing the empty list fs with the sorted list (in
    ...
    # you observed based on the produced figure.
    """
    fs = []
...
def w_i(data, word, width):
    ...
    # -----
    for s in data:
        positions = [i for i,x in enumerate(s) if x == word]
        for position in positions:
            for i in range (position - width, position + width +1):
                if i < 0 or i > len(s)-1 or i == position:
                    continue
                features.add(s[i])
    # -----
        ...
        for position in positions:
            for i in range (position - width, position + width +1):
                if i < 0 or i > len(s)-1 or i == position:
                    continue
                s_word.append(s[i])
        for ele in itertools.combinations(s_word , 2):
            features.add(ele)

    # for s in data:
    #     positions = [i for i,x in enumerate(s) if x == word]
    #     for position in positions:
    #         for i in range (position - width, position + width):
    #             if i < 0 or i> len(s)-2 or i == position:
    #                 continue
    #             for j in range (position-width + 1, position +width+1):
    #                 if j < 0 or j> len(s)-1 or j == position:
    #                     continue
    #                 features.add((s[i], s[j]))
    # -----
...
def context(data, word, width):
    ...
    # -----
    context = w_ij(data, word, width) | w_i(data, word, width)
    # -----
...
def similarity(cd, w1, w2):
    ...
    if w1==w2:
        ...
        sim = 0
    else: sim = 2 * len(cd[w1] & cd[w2]) / (len(cd[w1]) + len(cd[w2]))
    # -----
...
def get_knn_graph(cd, k):
    ...
    for word in cd:
        ...
        for w in cd:
            if w != word:
                sim_dict[w] = similarity(cd, word, w)
        ss = sorted(sim_dict, key = sim_dict.get, reverse = True)
        value = ss[:k]
        knng[word] = value
        # progress = 100.0*float(i)/len(cd.keys())
        # if i % 40 == 0:
        #     print ('*'*70)
...
def get_normalized_laplacian(sim_matrix):
    ...
    for i in range(0,100):
        for j in range(0,100):
            if i == j:
                degree_matrix[i][j] = sum(similarity_matrix[i])
            else: degree_matrix[i][j] = 0

    Laplacian = similarity_matrix - degree_matrix

    inv_d = np.linalg.inv(degree_matrix)
    l = np.matmul(inv_d, Laplacian)
