...
def main():
    ...
    graph = get_knn_graph(cd, 10)
    save_json('knng_10.json', graph)

    # This assignment is hard but I learned how to implement the code I learned
    # in class through completing it. The syntax of the words seem to be consistant
    # but the semantics of some words do not seem to be related to each other.
    #
    #
    # Such a graph of words, or word similarity in general can be useful for search 
    # engine optimization to show specific results to people who are looking for similar 
    # search results online. 
    # Also, it can be used to improve the relavance of the result that
    # the search engine returns to what the users are looking for.
    # 
    # Customer characters and human DNA can also be good cadidates. In marketing,
    # word similarity can be used to find customers who are in the same age,
    # gender, etc. Thus, we can target the right group of customers.
    # Human DNA consists of A, T, G, C so we can use word similarity to find people 
