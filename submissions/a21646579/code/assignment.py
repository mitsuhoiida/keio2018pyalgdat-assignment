...
def extract_words(sentence):
    ...
    # -----
    # YOUR CODE GOES HERE
    word = re.sub(r'[\W\d]', ' ', sentence)
    word = re.sub(r' [\w] ', ' ', word)
    word = word.lower()
    word = word.split()

    for x in word:
        if x != "re" and x != "ve" and x != "ll":
            words.append(x)
    # -----
...
def get_word_freq(data):
    ...
    for i in range(0, len(data)):
        for x in range(0, len(data[i])):
            data1.append(data[i][x])

    data = data1

    values = []
    for i in data:
        values.append(data.count(i))
...
def top_k_words(wfd, k):
    ...
    for key, values in sorted(wfd.items(), key=lambda x: -x[1]):
        top_k.append(str(key))
        top_k = top_k[:k]
    # -----
...
def w_i(data, word, width):
    ...
    for centence in data:
        if word in centence:
            for index, name in enumerate(centence):
                if index <= width:
                    features.extend(centence[:index + width + 1])
                elif index >= len(centence):
                    features.extend(centence[index - width:])
                else:
                    features.extend(centence[index - width:index + width + 1])
    ...
    for centence in data:
        if word in centence:
            for index, name in enumerate(centence):
                if word == name:
                    if index <= width:
                        feature = centence[:index + width + 1]
                    elif index >= len(centence):
                        feature = centence[index - width:]
                    else:
                        feature = centence[index - width:index + width + 1]
                    feature.remove(word)
...
def similarity(cd, w1, w2):
    ...
    if w1 == w2:
        ...
        numerator = len(cd[w1] & cd[w2])
        denominator = len(cd[w1] | cd[w2])
        if denominator > 0:
            sim = 2 * (abs(numerator) / abs(denominator))
    # -----
...
def get_knn_graph(cd, k):
    ...
    # -----
    # YOUR CODE GOES HERE
    for word in cd.items():
        number_cd = {}
        for word1 in word[1]:
            if len(word1) != 2:
                number_cd[word1] = similarity(cd, word[0], word1)
        sorted(number_cd.items(), key=lambda x: x[1])
        knng[word[0]] = list(number_cd.keys())[:k]
    # -----
...
def get_sim_matrix(cd):
    ...
    for w1 in cd.keys():
        loop2 = 0
        b1 = np.arange(len(cd.keys()))
        for w2 in cd.keys():
            b1[loop2] = similarity(cd, w1, w2)
            loop2 += 1
        word_index[w1] = b1
        sim_matrix[loop1] = b1
        loop1 += 1
