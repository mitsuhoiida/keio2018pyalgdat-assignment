...
def main():
    ...
    for i in words:
        for j in words:
            sim.append(similarity(cd, i, j))
    # 4
    f = open('top_100_sim.txt', 'w')
    for n, x in enumerate(sim):
        if (n + 1) % 100 == 0:
            f.write(str(x) + "\n")
        else:
            f.write(str(x) + ",")
    ...
    # For the empty corpus, it will return a zero matrix.
    # For the non-empty corpus with no words appearing, it will return a identity matrix
    # -----
    return
