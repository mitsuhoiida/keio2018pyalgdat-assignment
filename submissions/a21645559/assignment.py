...
# real vector space, it comes with a metric. This means we can measure distance
...
def extract_words(sentence):
    ...
    sentence2 = re.sub(r"\'\w*"," ", sentence)
    sentence3 = re.sub("[-,^.!:?;*$%&/]"," ", sentence2)
    words = re.sub("\d"," ", sentence3).split()
...
def load_corpus_data(file_name):
    ...
    with open(file_name, 'r') as fh:
        data = []
        for line in fh:
            data.append(extract_words(line))      
    # -----
...
def top_k_words(wfd, k):
    ...
    # -----
    word_sorted = sorted(wfd, key = wfd.__getitem__, reverse = True)
    for n in range(k):
        top_k.append(word_sorted[n])
    return(top_k)
    # -----
...
def graph_top_k(wfd, top_k, file_name):
    ...
    # -----
    for z in range(0, top_k):
        fs.append(wfd[top_k_words(wfd, top_k)[z]])
    # -----
...
def w_i(data, word, width):
    ...
    for sublist in data:
        if word in sublist:
            for idx, wd in enumerate(sublist):
                    if 0 < abs(idx - sublist.index(word)) <= width: 
                        features.add(sublist[idx])
    # -----
...
def w_ij(data, word, width):
    ...
    for sublist in data:
        if word in sublist:
            word_fit = list()
            for idx, wd in enumerate(sublist):
                if 0 < abs(sublist.index(word) - idx) <= width:
                    word_fit.append(sublist[idx])  
            features.add(tuple(itertools.combinations(word_fit, 2)))     
...
def context(data, word, width):
    ...
    # -----
    i = w_i(data, word, width)
    i_j = w_ij(data, word, width)    
    context = i.union(i_j)  
    # -----
...
def get_context_dictionary(data, words, width):
    ...
    for a in range(0, len(words)):
        cd[words[a]] = context(data, words[a], width)
        if a % 100 == 0:
            print(((a/len(words))*100))  
    # -----
...
def similarity(cd, w1, w2):
    ...
    if w1 == w2:
        ...
        sim = 0
    elif w1 not in list(cd.keys()):
        sim = 0
    elif w2 not in list(cd.keys()):
        sim = 0
    else:
        bunshi = (cd[w1]).intersection(cd[w2])
        bunbo = len(cd[w1]) + len(cd[w2])
        sim = 2 * (abs(len(bunshi))/abs(bunbo))
    # -----
...
## For this part, I understand that the codes I wrote are extremely inefficient,
## and will take forever. However, I could not come up with any other ways, so
## please grade what I have so far. For obtaining knng_10.json, I asked my 
## friend, Fetzek Calvin, to send it to me so that I can proceed to the next 
## step. 


def neighbors(words, cd, k):
    neighbors = list()
    whole_list = dict()
    for wds in cd.keys():
        whole_list[wds] = similarity(cd, words, wds)
    list_sorted = sorted(whole_list, key = whole_list.__getitem__, reverse = True)
    for a in range(1, k):
        neighbors.append(list_sorted[a])
    return neighbors
...
def get_knn_graph(cd, k):
    ...
    # -----
    for words in cd.keys():
        knng[words] = neighbors(words, cd, k)
    # -----
...
## I could not complete this part, so please ignore it. 
