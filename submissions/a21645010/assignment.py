...
def extract_words(sentence):
    ...
    list_of_words = list(filter(lambda w: len(w) > 1, re.split(r'\W',\
sentence.rstrip())))
    for vocabulary in list_of_words:
        vocabulary = re.sub(".*[\d].*", "", vocabulary)
        if len(vocabulary) > 1:
...
assignment/data/{}.txt'.format(file_name)) as f:
        for l in f:
...
def top_k_words(wfd, k):
    ...
    # -----
    i = 1
    for vocab, _ in sorted(wfd.items(), key=lambda x: -x[1]):
        if i > k:
            break
        top_k.append(vocab)
        i += 1
...
def w_i(data, word, width):
    ...
    for sentence in data:
        ...
        for i in range(0, len_sentence):
            if sentence[i] == word:
                for width_before in sentence[i-width:i]:
                    features.add(width_before)
                for width_after in sentence[i+1:i+1+width]:
                    features.add(width_after)
    # -----
...
def w_ij(data, word, width):
    ...
    for sentence in data:
        if sentence.count(word) > 0:
            indexes = [i for i, x in enumerate(sentence) if x == word]
            for ind in indexes:
                words_around_target_word = sentence[ind-width:ind] + (sentence[ind+1:(ind+1+width)])
                len_words_around_target_word = len(words_around_target_word)
                for i in range(len_words_around_target_word):
                    j = 1
                    while i+j < len_words_around_target_word:
                        features.add((words_around_target_word[i], words_around_target_word[i+j]))
                        j += 1
    # -----
...
def context(data, word, width):
    ...
    # -----
    wi = w_i(data, word, width)
    wij = w_ij(data, word, width)
    context = wi | wij
    # -----
...
def get_context_dictionary(data, words, width):
    ...
    # -----
    # i is only used for displaying the progress
    i = 0
    for vocabulary in words:
        cd[vocabulary] = context(data, vocabulary, width)
        i += 1
        if i % 500 == 0:
            print('{:.0%} done'.format(i / len(words)))
    # -----
...
def similarity(cd, w1, w2):
    ...
    if cd[w1] == cd[w2]:
        return 1
    elif len(cd[w1])*len(cd[w1]) == 0:
        return 0
    else:
        sim = 2 * len(cd[w1] & cd[w2]) / (len(cd[w1]) + len(cd[w2]))
    # -----
...
def get_knn_graph(cd, k):
    ...
    number_just_for_displaying_progress = 0
    len_cd_keys = len(cd.keys())

    # Main part
    for context in cd.keys():
        similarity_value = []
        for another_context in cd.keys():
            similarity_value.append(similarity(cd, context, another_context))
        word_context_combined = zip(similarity_value, cd.keys())
        j = 0
        nearest_words = []
        for _, b in sorted(word_context_combined, key = lambda t: t[0], reverse=True):
            nearest_words.append(b)
            j += 1
            if j > k:
                break
        nearest_words = nearest_words[1:]
        knng[context] = nearest_words

        # Below is just for showing progress.
        number_just_for_displaying_progress += 1
        if number_just_for_displaying_progress % 500 == 0:
            print('{:.0%} done'.format(number_just_for_displaying_progress / len_cd_keys))
    # -----
...
def get_sim_matrix(cd):
    ...
    # Preparation
    len_cd_keys = len(cd.keys())
    all_words_list = list(cd.keys())

    # For word_index
    for vocabulary in all_words_list:
        word_index[vocabulary] = all_words_list.index(vocabulary)
    
    len_cd_keys = len(cd.keys())
    for row in range(len_cd_keys):
        for column in range(len_cd_keys):
            if row == column:
                sim_matrix[row, column] = 1
            else:
                sim_matrix[row, column] = similarity(cd, all_words_list[row],\
all_words_list[column])
    # -----
...
def get_normalized_laplacian(sim_matrix):
    ...
    # -----
    # YOUR CODE GOES HERE
    diag = np.zeros(sim_matrix.shape)
    sums_for_rows = np.sum(sim_matrix, axis=0)
    i = 0
    for sums in sums_for_rows:
        diag[i,i] = sums
        i += 1
    l = diag - sim_matrix

    # Normalization
    l = np.matmul(np.linalg.inv(diag), l)
    # -----
...
def get_word_vectors(v, word_index):
    ...
    # -----
    # YOUR CODE GOES HERE
    for vocabulary, index in word_index.items():
        wv[vocabulary] = v[index]
    # -----
